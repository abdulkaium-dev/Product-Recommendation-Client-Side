    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 py-8 md:py-10">
      ...
      <div
        className="bg-gradient-to-r from-indigo-600 via-indigo-500 to-indigo-700
                   text-white p-6 sm:p-8 rounded-3xl shadow-lg mb-8
                   flex flex-col md:flex-row justify-between items-center gap-4"
      >
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-center md:text-left">
          My Product Queries
        </h2>
        <button
          onClick={() => navigate("/add-query")}
          className="w-full sm:w-auto bg-indigo-700 hover:bg-indigo-800 transition
                     text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-semibold
      ...
      >
        {queries.map((query) => (
          <article
            key={query._id}
            className="flex flex-col justify-between rounded-3xl shadow-xl
                       bg-gradient-to-tr from-indigo-50 via-indigo-50 to-pink-50
                       dark:from-indigo-900 dark:via-indigo-900 dark:to-pink-900
                       hover:from-indigo-100 hover:via-indigo-150 hover:to-pink-150
                       dark:hover:from-indigo-700 dark:hover:via-indigo-600 dark:hover:to-pink-800
                       transition-transform duration-300 hover:scale-105"
          >
            {/* Image */}
            <div className="w-full rounded-t-3xl overflow-hidden shadow-md bg-indigo-100 dark:bg-indigo-800 aspect-video min-h-[140px] max-h-[180px] sm:min-h-[220px]">
              <img
            ...
            <div className="p-5 flex flex-col flex-grow justify-between">
              <div>
                <h3
                  className="text-lg sm:text-xl md:text-2xl font-semibold mb-2 truncate
                             text-indigo-800 dark:text-indigo-200"
                  title={query.queryTitle || "Untitled Query"}
                >
                  {query.queryTitle || "Untitled Query"}
                </h3>
                <p className="text-indigo-700 dark:text-indigo-400 text-sm sm:text-base font-medium mb-1 truncate">
                  Brand: {query.productBrand || "N/A"}
                </p>
                <p className="text-indigo-600 dark:text-indigo-500 text-xs sm:text-sm mb-2">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-4">
                <button
                  ...
                  aria-label={`View query ${query.queryTitle || "Untitled"}`}
                  className="bg-white text-indigo-900 flex-1 px-4 py-2 rounded-xl shadow-md
                ...
                <button
                  ...
                  aria-label={`Update query ${query.queryTitle || "Untitled"}`}
                  className="bg-white text-indigo-900 flex-1 px-4 py-2 rounded-xl shadow-md
                             hover:bg-indigo-100 transition text-sm font-semibold"
                >
                  Update
                ...
                <button
                  ...
                  aria-label={`Delete query ${query.queryTitle || "Untitled"}`}
                  className="bg-indigo-600 text-white flex-1 px-4 py-2 rounded-xl shadow-md
