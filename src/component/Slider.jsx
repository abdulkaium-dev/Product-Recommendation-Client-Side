...
export default function HomeSlider() {
  ...
  return (
    <>
      ...
      >
        {slides.map(({ id, image, bgColor, titleTop, titleMain, titleHighlight }) => (
          <SwiperSlide key={id} aria-roledescription="slide">
            <section
              className={`${bgColor} relative w-full h-full flex items-center justify-center px-6 sm:px-10 lg:px-16`}
              aria-label={`${titleMain} featured slide`}
              role="group"
            >
              {/* Overlay for dark mode readability */}
              <div className="absolute inset-0 bg-black bg-opacity-30 dark:bg-opacity-60 transition-colors"></div>
              ...
              <div className="relative max-w-7xl w-full flex flex-col-reverse md:flex-row items-center justify-between h-full">
                ...
                <div className="text-white max-w-lg space-y-3 text-center md:text-left drop-shadow-md">
                  ...
                  <button
                    type="button"
                    className="mt-6 px-8 py-3 bg-primary text-white font-semibold rounded-md shadow-lg hover:bg-primary-hover hover:shadow-xl transition-transform transform hover:scale-105 duration-300 focus:outline-none focus:ring-4 focus:ring-primary"
      ...
      {/* Custom CSS for colors and dark mode */}
      <style jsx="true">{`
        :root {
